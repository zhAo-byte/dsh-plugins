...
选机器 → 选工作台 → 输入问题。整轮跑完后回答会一次性出现在页面上。
...
它也已经抓到过两个只有真浏览器才看得见的问题：首次打开时节点列表渲染出来却**没有自动选中**
（于是工作台、历史、发送按钮全是空的），以及乐观插入的问句和 SSE 推来的问句重复渲染成两条、
其中一条永远停在「等待中」。两个都修了，见 [`relay/public/index.html`](relay/public/index.html)。
它还会把三张截图落到临时目录，给人眼看。
...
| `workspaces` | `[]` | 允许访问的工作台；字符串或 `{ name, path }`。**不在列表里的一律拒绝** |
